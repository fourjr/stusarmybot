...
class Stats():
    ...
    async def profile(self, ctx, tag = None):
        ...
        async with ctx.channel.typing():
            ...
            crprof = await client.get_profile(tag)
            constants = await client.get_constants()
            if crprof == None:
                await ctx.send('Service down. Please be patient.')
                return

            chests = ''
            index = crprof.chest_cycle.position % len(constants.chest_cycle)
            chestindex = crprof.chest_cycle.position

            for i in range(10):
                if chestindex == crprof.chest_cycle.super_magical:
                    chests += emoji('chestsupermagical') + ' '
                elif chestindex == crprof.chest_cycle.legendary:
                    chests += emoji('chestlegendary') + ' '
                elif chestindex == crprof.chest_cycle.epic:
                    chests += emoji('chestepic') + ' '
                else:
                    chests += emoji('chest' + constants.chest_cycle[index].lower()) + ' '
                index += 1
                chestindex += 1
            ...
            for i in range(8):
                if crprof['currentDeck'][i]['name'] == 'Mini P.E.K.K.A':
                    ...
                    deck += f"{emoji(crprof['currentDeck'][i]['name'].lower().replace(' ', ''))}{crprof.deck[i].level}"
